...
const SECOND_OPINION_MODES = ["ask", "native-only", "strict"];
const DEFAULT_SETTINGS = { codex: true, ollama: true, adversarial: true, secondOpinionMode: "ask" };
...
const ADVERSARIAL_PROVIDER_ORDER = ["native", "ollama", "codex", "opencode", "kilo", "cursor", "antigravity"];
...
const REDACTION_TOKEN_ALLOWLIST = new Set([
  "structural-completeness-reviewer",
  "structural-completeness-reviewer.md",
  "structural-completeness-reviewer.toml",
]);
...
function readSettings() {
  ...
  return {
    ...settings,
    secondOpinionMode: normalizeSecondOpinionMode(settings.secondOpinionMode),
...
function normalizeModelValue(value) {
  ...
  return ["default", "unset"].includes(model.toLowerCase()) ? "" : model;
}
...
function normalizeReviewers(settings = {}) {
  ...
  const defaults = {
    ...
    codex: { enabled: settings.codex !== false, model: DEFAULT_REVIEWER_MODELS.codex },
      ...
      const enabled =
        typeof item.enabled === "boolean"
          ...
          : typeof defaults[name].enabled === "boolean"
            ...
            : false;
      const model = typeof item.model === "string" && item.model.trim() ? normalizeModelValue(item.model) : defaults[name].model;
...
function normalizeSecondOpinionMode(value) {
  const normalized = String(value || "").trim().toLowerCase();
  return SECOND_OPINION_MODES.includes(normalized) ? normalized : DEFAULT_SETTINGS.secondOpinionMode;
}

function secondOpinionPolicyStatus(settings = readSettings()) {
  const mode = normalizeSecondOpinionMode(settings.secondOpinionMode);
  return {
    mode,
    modes: SECOND_OPINION_MODES,
    externalReviewersEnabled: mode !== "native-only",
    approvalRequired: mode === "ask",
    strict: mode === "strict",
    notes:
      mode === "native-only"
        ? "Run native/local review only; configured external AI reviewer lanes are skipped."
        : mode === "strict"
          ? "Configured external AI reviewer lanes are required; blocked or failed lanes keep strict review or plan verification incomplete."
          : "Run native/local review first; external AI reviewer lanes are second opinions and should cross sandbox/privacy boundaries only after explicit approval.",
  };
}
...
function defaultAdversarialLaneId(provider) {
  return `${provider}-adversarial`;
}

function defaultAdversarialModel(provider, reviewers = {}) {
  const reviewerModel = reviewers[provider]?.model || "";
  if (reviewerModel) return reviewerModel;
  if (provider === "antigravity") return "default";
  return DEFAULT_REVIEWER_MODELS[provider] || "";
}

function splitAdversarialReviewersByProvider(settings = readSettings()) {
  const byProvider = new Map();
  const extras = [];
  for (const lane of effectiveAdversarialReviewers(settings)) {
    if (!lane || !ADVERSARIAL_PROVIDER_ORDER.includes(lane.provider)) continue;
    if (!byProvider.has(lane.provider)) {
      byProvider.set(lane.provider, lane);
    } else {
      extras.push(lane);
    }
  }
  return { byProvider, extras };
}
...
function adversarialReviewerSettingsStatus(options = {}) {
  ...
  return {
    ...
    reviewers: lanes.map((lane) => {
      ...
      const configuredModel = typeof lane.model === "string" ? normalizeModelValue(lane.model) : "";
      ...
      return {
        ...
        configuredModel,
        command,
        installed: lane.provider === "native" ? true : Boolean(installedPath),
        path: installedPath,
        ...
        notes:
          lane.provider === "native"
            ? "Runs in the active harness without forwarding context to an external CLI."
...
function addAdversarialReviewer(providerRaw, modelRaw, idRaw) {
  ...
  const rawModel = String(modelRaw || "").trim();
  if (!rawModel) {
    console.error("Usage: pza-runtime add-adversarial-reviewer <provider> <model> [id]");
    return 2;
  }
  const model = normalizeModelValue(rawModel);
  ...
  const id = explicitId ? normalizeLaneId(explicitId) : uniqueAdversarialLaneId(`${provider}-${model || "default"}`, existingIds);
  if (!id) {
...
function setAdversarialReviewer(idRaw, field, rawValue) {
  ...
  if (setting === "enabled") {
    ...
    lanes[index] = { ...lanes[index], model: normalizeModelValue(rawValue) };
  }
...
function reviewerSettingsStatus() {
  const settings = readSettings();
  const adversarialByProvider = splitAdversarialReviewersByProvider(settings).byProvider;
  const adversarialGlobalEnabled = settings.adversarial !== false;
  return REVIEWER_ORDER.map((name) => {
    ...
    const adversarialSupported = ADVERSARIAL_PROVIDER_ORDER.includes(name);
    const adversarialLane = adversarialSupported ? adversarialByProvider.get(name) || null : null;
    const adversarialEnabled = adversarialSupported && adversarialLane ? adversarialLane.enabled !== false : false;
    const adversarialEffectiveEnabled = adversarialGlobalEnabled && adversarialEnabled;
    const adversarialConfiguredModel = typeof adversarialLane?.model === "string" ? adversarialLane.model.trim() : "";
    const adversarialModel = adversarialConfiguredModel || defaultAdversarialModel(name, settings.reviewers);
    const adversarialPreflight = adversarialSupported
      ? reviewerPreflightStatus(name, {
          enabled: adversarialEffectiveEnabled,
          model: adversarialModel,
          installedPath,
          disabledBlocker: adversarialEnabled && !adversarialGlobalEnabled ? "Adversarial review is disabled." : "",
        })
      : { state: "unavailable", blocker: "Native review cannot run as an external adversarial reviewer." };
    return {
      name,
      label: REVIEWER_LABELS[name] || name,
      enabled,
      model,
      adversarialSupported,
      adversarialConfigured: Boolean(adversarialLane),
      adversarialEnabled,
      adversarialEffectiveEnabled,
      adversarialLaneId: adversarialLane?.legacy ? defaultAdversarialLaneId(name) : adversarialLane?.id || defaultAdversarialLaneId(name),
      adversarialModel,
      adversarialConfiguredModel,
      defaultAdversarialModel: defaultAdversarialModel(name, settings.reviewers),
      adversarialState: adversarialPreflight.state,
      adversarialBlocker: adversarialPreflight.blocker,
      adversarialLegacy: adversarialLane?.legacy === true,
      command,
...
function printSecondOpinionPolicy() {
  console.log(JSON.stringify({ path: SETTINGS_PATH, secondOpinion: secondOpinionPolicyStatus() }, null, 2));
}

function setSecondOpinionMode(modeRaw, print = true) {
  const raw = String(modeRaw || "").trim().toLowerCase();
  if (!SECOND_OPINION_MODES.includes(raw)) {
    console.error(`Second-opinion mode must be one of: ${SECOND_OPINION_MODES.join(", ")}.`);
    return 2;
  }
  writeSettings({ secondOpinionMode: raw });
  if (print) printSecondOpinionPolicy();
  return 0;
}
...
function saveReviewerUiState(payload) {
  ...
  for (const name of REVIEWER_ORDER) {
    ...
    if (typeof value.model === "string") reviewers[name].model = normalizeModelValue(value.model);
  }
  ...
  if (incomingAdversarial) {
    ...
    for (const lane of incomingAdversarial) {
      ...
      adversarialReviewers.push({
        id,
        provider,
        model: typeof lane.model === "string" ? normalizeModelValue(lane.model) : "",
        enabled: lane.enabled !== false,
      });
    }
  }
  ...
  if (typeof payload.secondOpinionMode === "string") next.secondOpinionMode = normalizeSecondOpinionMode(payload.secondOpinionMode);
...
function settingsUiState() {
  const settings = readSettings();
  const adversarialSplit = splitAdversarialReviewersByProvider(settings);
  return {
    paths: { settings: SETTINGS_PATH, model: MODEL_PATH },
    secondOpinion: secondOpinionPolicyStatus(settings),
    adversarial: settings.adversarial !== false,
    ...
    extraAdversarialReviewers: adversarialSplit.extras,
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .table-head, .row {
      display: grid;
      grid-template-columns: 1.25fr 96px 96px 120px 96px minmax(200px, 1.25fr) 1.35fr;
      gap: 12px;
      align-items: center;
      padding: 12px 14px;
    }
    ...
    @media (max-width: 800px) {
      ...
      .adversarial-toggle::before { content: "Adversarial"; }
...
<body>
  <main>
    <header>
      <div>
        <h1>PZA Settings</h1>
        <p>Configure reviewer backends, second-opinion policy, and adversarial review toggles used by /arewedone.</p>
      </div>
      <div class="actions">
    ...
    <section class="adversarial">
      <div>
        <strong>Second-opinion mode</strong>
        <p>Controls how external AI reviewer lanes cross sandbox and privacy boundaries.</p>
      </div>
      <select id="secondOpinionMode" aria-label="Second-opinion mode">
        <option value="ask">Ask before external review</option>
        <option value="native-only">Native review only</option>
        <option value="strict">Require external review</option>
      </select>
    </section>
  ...
  <script>
    ...
    const secondOpinionModeEl = document.getElementById("secondOpinionMode");
    ...
    function reviewerNote(reviewer) {
      ...
      if (reviewer.adversarialEnabled) {
        if (reviewer.adversarialBlocker) parts.push("Adversarial: " + reviewer.adversarialBlocker);
        if (reviewer.adversarialLegacy) parts.push("Adversarial uses the legacy default lane until settings are saved.");
        if (reviewer.adversarialModel && reviewer.adversarialModel !== reviewer.model) {
          parts.push("Adversarial model: " + reviewer.adversarialModel + ".");
        }
      }
    ...
    function render(data) {
      state = data;
      secondOpinionModeEl.value = data.secondOpinion && data.secondOpinion.mode ? data.secondOpinion.mode : "ask";
      ...
      for (const reviewer of data.reviewers) {
        ...
        adversarial.className = "adversarial-toggle";
        if (reviewer.adversarialSupported) {
          row.dataset.adversarialSupported = "true";
          row.dataset.adversarialConfigured = reviewer.adversarialConfigured ? "true" : "false";
          row.dataset.adversarialLaneId = reviewer.adversarialLaneId || reviewer.name + "-adversarial";
          row.dataset.adversarialModel = reviewer.adversarialConfiguredModel || reviewer.adversarialModel || "";
          row.dataset.defaultAdversarialModel = reviewer.defaultAdversarialModel || "";
          const advLabel = document.createElement("label");
          advLabel.className = "toggle";
          const advCheckbox = document.createElement("input");
          advCheckbox.type = "checkbox";
          advCheckbox.checked = Boolean(reviewer.adversarialEnabled);
          advCheckbox.dataset.field = "adversarialEnabled";
          const advToggleText = document.createElement("span");
          advToggleText.textContent = "On";
          advLabel.append(advCheckbox, advToggleText);
          const advState = document.createElement("span");
          advState.className = "pill " + statePillClass(reviewer.adversarialState);
          advState.textContent = reviewer.adversarialState || "unknown";
          adversarial.append(advLabel, advState);
        } else {
          row.dataset.adversarialSupported = "false";
          const unsupported = document.createElement("span");
          unsupported.className = "pill";
          unsupported.textContent = "N/A";
          adversarial.append(unsupported);
        }
        ...
        if (
          reviewer.blocker ||
          reviewer.adversarialBlocker ||
          (reviewer.enabled && !reviewer.installed) ||
          (reviewer.adversarialEnabled && !reviewer.installed)
        ) {
          notes.classList.add("warning");
        }

        row.append(name, installed, enabled, adversarial, reviewerState, model, notes);
    ...
    function collect() {
      ...
      const providerAdversarialEnabled = {};
      for (const row of reviewersEl.querySelectorAll(".row")) {
        const name = row.dataset.name;
        const model = row.querySelector('[data-field="model"]').value.trim();
        const adversarialCheckbox = row.querySelector('[data-field="adversarialEnabled"]');
        const adversarialEnabled = adversarialCheckbox ? adversarialCheckbox.checked : false;
        providerAdversarialEnabled[name] = adversarialEnabled;
        reviewers[row.dataset.name] = {
          enabled: row.querySelector('[data-field="enabled"]').checked,
          model,
        };
        if (row.dataset.adversarialSupported === "true" && (adversarialEnabled || row.dataset.adversarialConfigured === "true")) {
          const laneId = row.dataset.adversarialLaneId || name + "-adversarial";
          adversarialReviewers.push({
            id: laneId,
            provider: name,
            model: model || row.dataset.adversarialModel || row.dataset.defaultAdversarialModel || "",
            enabled: adversarialEnabled,
          });
          adversarialIds.add(laneId);
        }
      }
      for (const lane of state.extraAdversarialReviewers || []) {
        if (!lane || adversarialIds.has(lane.id)) continue;
        adversarialReviewers.push({
          id: lane.id,
          provider: lane.provider,
          model: lane.model || "",
          enabled: providerAdversarialEnabled[lane.provider] === false ? false : lane.enabled !== false,
        });
        adversarialIds.add(lane.id);
      }
      const checks = {};
      ...
      const payload = {
        secondOpinionMode: secondOpinionModeEl.value,
        adversarial: adversarialReviewers.some((lane) => lane.enabled),
        reviewers,
        checks,
...
function isRedactionTokenAllowed(value) {
  const token = String(value || "");
  return REDACTION_TOKEN_ALLOWLIST.has(token);
}

function looksLikeSafePathToken(value) {
  const token = String(value || "");
  if (!token.includes("/") || /[+=]/.test(token)) return false;
  if (/^[A-Za-z][A-Za-z0-9+.-]*:\/\//.test(token) || token.startsWith("//")) return false;
  const segments = token.split(/[\\/]+/).filter(Boolean);
  for (const segment of segments) {
    if (segment.length >= 32) {
      if (looksLikeHashOrUuid(segment)) return false;
      if (/\.[A-Za-z0-9]{1,8}$/.test(segment)) continue;
      const uniqueChars = new Set(segment).size;
      if (uniqueChars >= 10 && shannonEntropy(segment) >= 3.6) return false;
    }
  }
  return true;
}
...
function redactContext(input) {
  ...
  return output.replace(/(^|[^A-Za-z0-9_./+-])([A-Za-z0-9_./+-]{32,}={0,2})(?=$|[^A-Za-z0-9_./+=-])/g, (match, prefix, token) => {
    if (isRedactionTokenAllowed(token)) return `${prefix}${token}`;
    if (looksLikeHashOrUuid(token)) return `${prefix}${token}`;
    if (looksLikeSafePathToken(token)) return `${prefix}${token}`;
    const uniqueChars = new Set(token).size;
    const redacted = uniqueChars >= 10 && shannonEntropy(token) >= 3.6 ? "[REDACTED_HIGH_ENTROPY_TOKEN]" : token;
...
function printSkillStatus(skill) {
  ...
  if (skill === "arewedone" || skill === "pza-settings" || !skill) {
    status.secondOpinion = secondOpinionPolicyStatus();
...
function currentDiffHash() {
  ...
  try {
    ...
    const files = splitNul(output.toString("utf8")).sort();
...
function gitPathList(args) {
  return splitNul(gitOutput([...args, "-z"]));
}

function diffPathState(files, diffArgs) {
  const state = {};
  for (const file of files) {
    state[file] = sha256(gitOutput([...diffArgs, "--", file], { maxBuffer: 10 * 1024 * 1024 }));
  }
  return state;
}

function untrackedPathState(files) {
  const state = {};
  for (const file of files) {
    try {
      const stat = fs.statSync(file);
      if (stat.isFile()) {
        state[file] = `file:${stat.size}:${sha256(fs.readFileSync(file))}`;
      } else if (stat.isDirectory()) {
        state[file] = "directory";
      } else {
        state[file] = `other:${stat.mode}:${stat.size}`;
      }
    } catch (error) {
      state[file] = `unreadable:${error.code || error.message}`;
    }
  }
  return state;
}
...
function currentWorktreeState() {
  const unstagedFiles = gitPathList(["diff", "--name-only"]);
  const stagedFiles = gitPathList(["diff", "--cached", "--name-only"]);
  const untrackedFiles = gitPathList(["ls-files", "--others", "--exclude-standard"]);
  return {
    unstaged: diffPathState(unstagedFiles, ["diff"]),
    staged: diffPathState(stagedFiles, ["diff", "--cached"]),
    untracked: untrackedPathState(untrackedFiles),
    statusShort: gitOutput(["status", "--short"]),
  };
}

function currentDiffSnapshot() {
  return { hash: currentDiffHash(), state: currentWorktreeState() };
}

function comparePathState(before = {}, after = {}) {
  const paths = uniqueSorted([...Object.keys(before), ...Object.keys(after)]);
  return paths
    .map((file) => {
      if (before[file] === after[file]) return null;
      if (before[file] === undefined) return { file, change: "added" };
      if (after[file] === undefined) return { file, change: "removed" };
      return { file, change: "changed" };
    })
    .filter(Boolean);
}

function appendChangeSection(lines, label, changes, limit = 20) {
  if (!changes.length) return false;
  lines.push(`- ${label}:`);
  for (const item of changes.slice(0, limit)) {
    lines.push(`  ${item.change}: ${item.file}`);
  }
  if (changes.length > limit) lines.push(`  ... ${changes.length - limit} more`);
  return true;
}

function formatWorktreeChangeDetails(beforeSnapshot, afterSnapshot) {
  const before = beforeSnapshot.state || {};
  const after = afterSnapshot.state || {};
  const lines = ["PZA worktree-change details:"];
  let hasPathDetails = false;
  hasPathDetails = appendChangeSection(lines, "unstaged tracked paths changed during review", comparePathState(before.unstaged, after.unstaged)) || hasPathDetails;
  hasPathDetails = appendChangeSection(lines, "staged tracked paths changed during review", comparePathState(before.staged, after.staged)) || hasPathDetails;
  hasPathDetails = appendChangeSection(lines, "untracked paths changed during review", comparePathState(before.untracked, after.untracked)) || hasPathDetails;
  if (!hasPathDetails) {
    lines.push("- Diff hash changed, but path-level comparison did not identify a changed tracked, staged, or untracked path.");
  }
  const statusLines = splitDisplayLines(after.statusShort);
  if (statusLines.length) {
    lines.push("- Current git status --short:");
    for (const line of statusLines.slice(0, 30)) lines.push(`  ${line}`);
    if (statusLines.length > 30) lines.push(`  ... ${statusLines.length - 30} more`);
  }
  return lines.join("\n");
}
...
function runReviewer(modeRaw, providerRaw, modelRaw, prompt) {
  ...
  if (provider === "native") {
    const outcome = staticReviewerOutcome(
      "blocked",
      2,
      "native review runs inside the active harness; use the native reviewer agent instead of run-reviewer",
    );
  ...
  const beforeSnapshot = currentDiffSnapshot();
  ...
  const afterSnapshot = currentDiffSnapshot();
  ...
  if (beforeHash !== afterHash) {
    ...
    console.error(formatWorktreeChangeDetails(beforeSnapshot, afterSnapshot));
...
if (cmd === "settings") printStatus();
  ...
  const model = normalizeModelValue(process.argv[3]);
  writeModel(model);
  setReviewer("ollama", "model", model, false);
  console.log(readModel());
} else if (cmd === "reviewer-settings") printReviewerSettings();
else if (cmd === "check-settings") printCheckSettings();
else if (cmd === "second-opinion-policy") printSecondOpinionPolicy();
...
} else if (cmd === "set-second-opinion-mode") {
  process.exit(setSecondOpinionMode(process.argv[3]));
  ...
  console.error("Usage: pza-runtime <settings|get-setting|set-settings|get-model|set-model|reviewer-settings|check-settings|second-opinion-policy|set-second-opinion-mode|adversarial-reviewer-settings|add-adversarial-reviewer|set-adversarial-reviewer|remove-adversarial-reviewer|set-reviewer|set-check|get-reviewer-enabled|get-reviewer-model|settings-ui|plan-reviewers|skill-status|redact-context|collect-plan-context|collect-review-context|validate-hook-proposal|plan-review-prompt|run-plan-reviewer|session-files|session-stat|mark-reviewed|diff-hash|ollama-run|run-reviewer|run-check>");
  process.exit(2);
}
