...
- Review marker files: `/arewedone` writes `/tmp/pza-skills-session-<id>-reviewed.json` on completion. Compatibility `/ollama-review` may also write the marker for older installs. The `review-reminder` Stop hook also reads legacy marker paths during migration.
