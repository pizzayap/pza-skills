...
- Plan-review CLI prompt pattern: write or materialize the plan to a temp file, then run `node "$HOME/.pza-skills/lib/pza-runtime.js" plan-review-prompt "$PLAN_FILE" "$PLAN_SOURCE" > "$PROMPT_FILE"` and pipe `"$PROMPT_FILE"` to the reviewer. Custom reviewers must run through `node "$HOME/.pza-skills/lib/pza-runtime.js" run-plan-reviewer <name>` so commands are executed as argv arrays from `~/.pza-skills/plan-reviewers.json`.
