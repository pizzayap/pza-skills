  ...
  return {
    ...
    reviewers: lanes.map((lane) => {
      ...
      return {
        ...
        enabled: lane.enabled !== false,
        effectiveEnabled: globalEnabled && lane.enabled !== false,
        model,
        configuredModel,
        command,
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .lane-head, .lane-row {
      display: grid;
      grid-template-columns: minmax(150px, 1fr) 150px minmax(220px, 1.5fr) 90px minmax(170px, 1fr) 90px;
      gap: 12px;
      align-items: center;
      padding: 12px 14px;
    }
...
<body>
  ...
  <script>
    ...
    function renderAdversarialLane(lane) {
      ...
      if (lane.enabled && !lane.installed) notes.classList.add("warning");
      ...
      row.append(id, provider, model, enabled, notes, action);
      return row;
    }
    ...
    function collect() {
      ...
      const payload = { adversarial: adversarialEl.checked, reviewers };
...
function printSkillStatus(skill) {
  const status = {
    ...
    reviewers: reviewerSettingsStatus(),
    adversarialReviewers: adversarialReviewerSettingsStatus().reviewers,
    planReviewers: readPlanReviewers().map(({ name, enabled, index }) => ({ name, enabled, index, commandConfigured: true })),
    cliAvailability: cliAvailabilityStatus(),
  };
  if (skill === "arewedone") {
...
function runSpawnedReviewer(provider, args, prompt, options = {}) {
  ...
  if (!command || !commandPath(command)) {
    console.error(`${REVIEWER_LABELS[provider] || provider} ${options.label} skipped - ${command || provider} is not installed.`);
    return 127;
  }
  ...
  if (result.error) {
    ...
    console.error(`${REVIEWER_LABELS[provider] || provider} ${options.label} skipped - ${reason}`);
    return result.error.code === "ENOENT" ? 127 : 1;
  }
  if ((result.status ?? 1) !== 0 && reviewerResultLooksUnauthenticated(output)) {
    console.error(`${REVIEWER_LABELS[provider] || provider} ${options.label} skipped - not authenticated.`);
  }
  return result.status ?? 1;
}
...
function runReviewer(modeRaw, providerRaw, modelRaw, prompt) {
  ...
  if (!REVIEWER_COMMANDS[provider]) {
    console.error(`Reviewer skipped - unsupported provider: ${providerRaw}`);
    return 2;
  }
  ...
  if (provider === "ollama") {
    if (!model) {
      console.error("Ollama review skipped - model is not configured.");
      exitCode = 2;
    } else if (!commandPath(REVIEWER_COMMANDS.ollama)) {
      console.error("Ollama review skipped - Ollama is not installed.");
      exitCode = 127;
    } else {
      exitCode = runOllama(model, prompt);
    }
  } else if (provider === "codex") {
    const args = ["exec"];
    if (model) args.push("--model", model);
    args.push("-");
    exitCode = runSpawnedReviewer(provider, args, prompt, { label, stdin: true });
  } else if (provider === "opencode") {
    const args = ["run"];
    if (model) args.push("--model", model);
    args.push(prompt);
    exitCode = runSpawnedReviewer(provider, args, prompt, { label });
    ...
    if (!antigravitySupportsSafePrint()) {
      console.error("Antigravity review skipped - safe non-interactive sandbox print mode was not confirmed.");
      exitCode = 2;
    } else {
      exitCode = runSpawnedReviewer(provider, ["--sandbox", "--print", prompt], prompt, { label });
    }
  }
...
if (cmd === "settings") printStatus();
  ...
  console.error("Usage: pza-runtime <settings|get-setting|set-settings|get-model|set-model|reviewer-settings|adversarial-reviewer-settings|add-adversarial-reviewer|set-adversarial-reviewer|remove-adversarial-reviewer|set-reviewer|get-reviewer-enabled|get-reviewer-model|settings-ui|plan-reviewers|skill-status|redact-context|collect-plan-context|collect-review-context|validate-hook-proposal|plan-review-prompt|run-plan-reviewer|session-files|session-stat|mark-reviewed|diff-hash|ollama-run|run-reviewer>");
  process.exit(2);
}
