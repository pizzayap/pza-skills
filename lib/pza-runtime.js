...
function printStatus() {
  ...
  console.log(JSON.stringify({ settings, model: readModel(), paths: { settings: SETTINGS_PATH, model: MODEL_PATH, planReviewers: PLAN_REVIEWERS_PATH } }, null, 2));
}
...
if (cmd === "settings") printStatus();
  ...
  console.error("Usage: pza-runtime <settings|get-setting|set-settings|get-model|set-model|plan-reviewers|plan-review-prompt|run-plan-reviewer|session-files|session-stat|mark-reviewed|diff-hash|ollama-run>");
  process.exit(2);
}
