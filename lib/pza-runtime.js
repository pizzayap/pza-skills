...
if (cmd === "settings") printStatus();
  ...
  console.error("Usage: pza-runtime <settings|get-setting|set-settings|get-model|set-model|reviewer-settings|adversarial-reviewer-settings|add-adversarial-reviewer|set-adversarial-reviewer|remove-adversarial-reviewer|set-reviewer|get-reviewer-enabled|get-reviewer-model|settings-ui|plan-reviewers|plan-review-prompt|run-plan-reviewer|session-files|session-stat|mark-reviewed|diff-hash|ollama-run>");
  process.exit(2);
}
