...
description: >-
  ...
  quality review, AND Ollama code review in parallel, then synthesizes findings.
user-invocable: true
---
...
**If an agent fails or returns an error:** Still synthesize results from the remaining agents. Mark the failed agent's verdict as "skipped" in the summary table and note the error. Two out of three successful reviews still provide useful signal. If all three fail, report the errors and stop.
