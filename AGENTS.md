...
- If sandbox approval for an external reviewer is denied because private workspace context would leave the machine, mark that enabled reviewer lane `blocked` and continue with local review, proof commands, and static scans, but do not declare strict review complete. Do not route around the denial.
